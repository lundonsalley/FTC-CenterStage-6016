...
public class BlueAutonomousBackdrop extends LinearOpMode {
    ...
    private static final String TFOD_MODEL_FILE = "Pawn.tflite";
    ...
    public void runOpMode() throws InterruptedException {
        ...
        slideMotor = hardwareMap.get(DcMotorEx.class, Config.Hardware.Motor.slideMotorName); //port 1
        ...
        clawServo.setPosition(Config.Hardware.Servo.clawClosedPosition);
        ...
        while(opModeIsActive()) {
            if((PawnDetection()<1 || getRuntime()<7) && getRuntime()<7) {

                wristServo.setPosition(Config.Hardware.Servo.wristServoLow);
                clawServo.setPosition(Config.Hardware.Servo.clawClosedPosition);
    ...
    public void configAutoNav(){
        if(left){
            autoNav.claw(Config.Hardware.Servo.clawOpenPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);
            autoNav.move(new Position(DistanceUnit.METER,-0.85*convert,1.6*convert,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.claw(Config.Hardware.Servo.clawOpenPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);//drop pixel
            armMotor.setPower(0);
            armMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            armMotor.setMode(DcMotorEx.RunMode.RUN_TO_POSITION);
            autoNav.arm(100,0.3,armMotor);
            autoNav.move(new Position(DistanceUnit.METER,0.4*convert,-0.1*convert,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.arm(0,0.2,armMotor);
            autoNav.sleep(100);
            autoNav.move(new Position(DistanceUnit.METER,0,0.1*convert,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.claw(Config.Hardware.Servo.clawClosedPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);//close claw
            autoNav.sleep(200);
            autoNav.arm(100,0.3,armMotor);
            autoNav.move(new Position(DistanceUnit.METER,0,-1.6*convert,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.move(new Position(DistanceUnit.METER,-2.6*convert,0,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.claw(Config.Hardware.Servo.clawOpenPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);
        }else if(center){
            autoNav.claw(Config.Hardware.Servo.clawOpenPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);autoNav.move(new Position(DistanceUnit.METER,0,1.9*convert,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.claw(Config.Hardware.Servo.clawOpenPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);//drop pixel
            armMotor.setPower(0);
            armMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            armMotor.setMode(DcMotorEx.RunMode.RUN_TO_POSITION);
            ...
            autoNav.arm(100,0.3,armMotor);
            autoNav.move(new Position(DistanceUnit.METER,0,-1.6*convert,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.move(new Position(DistanceUnit.METER,-4*convert,0,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.claw(Config.Hardware.Servo.clawOpenPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);
        }else {
            autoNav.claw(Config.Hardware.Servo.clawOpenPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);
            autoNav.move(new Position(DistanceUnit.METER,0.9*convert,1.8*convert,0, 500),frontLeftMotor, frontRightMotor, backLeftMotor, backRightMotor);
            autoNav.claw(Config.Hardware.Servo.clawOpenPosition,clawServo,Config.Hardware.Servo.wristServoLow,wristServo);//drop pixel
            armMotor.setPower(0);
            armMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            armMotor.setMode(DcMotorEx.RunMode.RUN_TO_POSITION);
